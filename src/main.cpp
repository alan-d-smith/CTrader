...
int main() {
    ...
    auto append_log = [&](const std::string& msg, LogLevel level = LogLevel::Info) {
        ...
        log_lines.push_back({std::string("[") + ts + "] " + msg, level});
    ...
    auto do_fetch = [&]
    {
        ...
        for (const auto& sym : syms_copy) {
            ...
            if (!ok) {
                ...
                if (http_status == 403 || http_status == 429) {
                    ...
                    err_msg = "HTTP " + std::to_string(http_status) + " - " + body;
                } else if (!error.empty()) {
                    err_msg = "cURL " + std::to_string(curl_code) + " - " + error;
                } else {
        ...
        if (ok_all) {
            ...
            last_request_ok = false;
            last_error = err_msg;
            if (prev_quotes_ok) append_log("Quotes error: " + err_msg, LogLevel::Error);
        ...
        for (const auto& interval : order) {
            ...
            if (!R.ok) {
                if (R.http_status != 429) {
                    err_here = !R.body.empty()
                        ? "HTTP " + std::to_string(R.http_status) + " - " + R.body
                        : "cURL " + std::to_string(R.curl_code) + " - " + R.error;
                }
            } else {
                ...
                if (C.ok && !C.t.empty()) {
                    ...
                    err_here = "Yahoo candles: " + (C.err.empty() ? std::string("no_data") : C.err);
                }
            }
            ...
            if (interval == selected_interval) {
                debug_info.url = url;
                debug_info.http_status = R.http_status;
                debug_info.curl_code = R.curl_code;
                debug_info.curl_error = R.error;
                debug_info.body_snippet = R.body.size() > 2000 ? R.body.substr(0,2000)+"...(truncated)" : R.body;
                ...
                if (ok_here) {
                    ...
                    last_request_ok = false;
                    last_error = err_here;
                    if (prev_chart_ok) append_log("Chart error (" + interval + "): " + err_here, LogLevel::Error);
                    prev_chart_ok = false;
                }
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        bool symbol_changed = false;
        {
            ...
            draw_quotes_window(symbols,
                    last_request_ok, last_error, quotes, &debug_info, last_sync, log_lines,
